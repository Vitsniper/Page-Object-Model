...
class UIObject:
    ...
    def get_element(self, wait=10):
        ...
        return Browser.browser_instance.find_element(self.by, self.locator)

    def get_all_elements(self, wait=10):
        self.wait_to_appear(wait)
        return Browser.browser_instance.find_elements(self.by, self.locator)
    ...
    def is_checked(self):
        return Browser.browser_instance.execute_script("return arguments[0].checked;", self.get_element())

    def exists(self):
        try:
            WebDriverWait(Browser.browser_instance, 1).until(EC.presence_of_element_located((self.by, self.locator)))
            return True
        except:
            return False
    ...
    def is_clickable(self):
        def is_clickable(by, locator):
            try:
                WebDriverWait(Browser.browser_instance, 1).until(EC.element_to_be_clickable((by, locator)))
                return True
            except:
                return False
    ...
    def wait_to_disappear(self, seconds=10, ignore_error=False):
        ...
        while (time.time() - start) < seconds:
            try:
                WebDriverWait(Browser.browser_instance, seconds).until(
    ...
    def click(self, wait=10, use_action_chains=False):
        ...
        initial_handles = Browser.browser_instance.window_handles

        if use_action_chains:
            ui_object = self.get_element()
            ActionChains(Browser.browser_instance).move_to_element(ui_object).click().perform()
        else:
            try:
        ...
        if len(Browser.browser_instance.window_handles) > len(initial_handles):
    ...
    def scrollIntoCenter(self):
        ...
        Browser.browser_instance.execute_script(scrollElementIntoMiddle, self.get_element())

    def scrollIntoView(self):
        Browser.browser_instance.execute_script("arguments[0].scrollIntoView()", self.get_element())
        return self
    ...
    def press_key(self, key, use_action_chains=False):
        if not use_action_chains:
            ...
            chains = ActionChains(driver=Browser.browser_instance)
            chains.send_keys(key).perform()
        return self

    def mouse_over(self):
        ui_object = self.get_element()
        ActionChains(Browser.browser_instance).move_to_element(ui_object).perform()
        return self